...
import com.ekr.jularis.data.cart.ProductCart
...
class CartAdapter(val context: Context, private var checkout: ArrayList<Checkout>,var boolean: Boolean) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.bind(
            checkout[position],
            checkout[position].pictureCart.picture,
            checkout[position].productCart,
            boolean
        )
    ...
    class ViewHolder(itemView: View, private val listener: OnItemClickListener) :
        ...
        fun bind(checkout: Checkout, picture: String, productCart: ProductCart, boolean: Boolean) {
            val position = adapterPosition
            var cekLis = boolean
            val cent = checkout.checked
            with(itemView) {
                ...
                tv_title_item_keranjang.text = productCart.name
